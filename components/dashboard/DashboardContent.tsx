...
import { Plus, Eye, Trash2, Calendar, Users, LayoutTemplate, Clock, AlertTriangle, Loader2 } from "lucide-react";
...
export default function DashboardContent({
  ...
  const now = new Date();
  const isExpired = initialShop.planEndDate ? new Date(initialShop.planEndDate) < now : false;
  const isActuallyExpired = isExpired || !initialShop.isActive;

  const getDaysRemaining = (planEndDate: string | Date | null) => {
  ...
  const daysRemaining = getDaysRemaining(initialShop.planEndDate);
  ...
  const invitationsThisMonth = invitations.filter((inv) => {
    const invDate = new Date(inv.createdAt);
    return invDate.getMonth() === now.getMonth() && invDate.getFullYear() === now.getFullYear();
  }).length;

  const limitLabel = initialShop.plan === "premium" ? "Unlimited" : (initialShop.plan === "standard" ? "75" : "30");
  const invitationsLimit = initialShop.plan === "premium" ? 999999 : (initialShop.plan === "standard" ? 75 : 30);
  const percentUsed = invitationsLimit > 0 ? (invitationsThisMonth / invitationsLimit) * 100 : 0;

  const limitReached = !isActuallyExpired && initialShop.plan !== "premium" && percentUsed >= 100;
  const showWarning = !isActuallyExpired && initialShop.plan !== "premium" && percentUsed >= 80 && percentUsed < 100;

  const handleDelete = async (id: string) => {
    if (!confirm("இந்த அழைப்பிதழை நிச்சயமாக நீக்க விரும்புகிறீர்களா? (Are you sure you want to delete this invitation?)")) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(errData.error || "அழைப்பிதழை நீக்க முடியவில்லை");
      }
    } catch (err) {
      console.error(err);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center">
              <h1 className="text-xl font-bold text-gray-900">{initialShop.shopName}</h1>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="flex items-baseline">
                      <div className="text-2xl font-semibold text-gray-900 capitalize">{initialShop.plan}</div>
                    </dd>
                  </dl>
                </div>
              </div>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                      <div className="flex justify-center items-center gap-2">
                        ...
                        <span>பதிவேற்றப்படுகிறது... (Loading...)</span>
                      </div>
                    </td>
                  </tr>
